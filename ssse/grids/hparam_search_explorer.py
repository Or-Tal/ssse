...
def explorer(launcher):
    ...
    launcher.slurm_(gpus=4, partition='learnlab', cpus_per_gpu=1, time=2880, comment='v0.01')
    launcher.bind_({
    ...
    with launcher.job_array():
        ...
        for c_factor, reg_factor in product([0.05, 0.1, 0.3, 1, 2, 5, 10], [0.05, 0.1, 0.3, 1, 2, 5, 10]):
            sub({'loss.contrastive_factor': c_factor, 
                'loss.noise_regularization_factor': reg_factor, 
                'wandb.name': f"hparam_search_c{str(c_factor).replace('.', '_')}_reg{str(reg_factor).replace('.', '_')}",
                })